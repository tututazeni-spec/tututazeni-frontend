// components/scalability/NewIntegrationModal.tsx
// Modal "Nova Integração" — separador Integrações do módulo de Escalabilidade.
// Formulário completo (ligação, autenticação, sincronização, confiabilidade,
// gestão) mapeado 1:1 para CreateIntegrationConfigDto
// (src/scalability/scalability.dto.ts) — POST /scalability/integrations.
//
...
// ScalabilityService.buildCredentialsJson). "Testar conexão" chama
// POST /scalability/integrations/test-connection, que não precisa de a
// integração já existir.
...
export function NewIntegrationModal({ tenantId, onClose }: NewIntegrationModalProps) {
  ...
  const [status, setStatus] = useState<IntegrationStatus>('CONFIGURING');
  ...
  const handleTestConnection = async () => {
    ...
    const result = await apiClient
      .post<TestConnectionResult>('/scalability/integrations/test-connection', {
      ...
      >
        <div className="mt-5 flex flex-col gap-6">
          ...
          <Section title="Ligação">
            <div className="sm:col-span-2">
              <FormField label="URL/Endpoint" htmlFor="ni-baseurl">
                <Input
                  id="ni-baseurl"
                  value={baseUrl}
