        ...
        <>
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
              <KpiCard
                label="Inscritos"
                value={data.registration.registered}
                intent="primary"
              />
              <KpiCard
                label="Taxa de inscrição"
                value={`${data.registration.rate}%`}
                intent="info"
              />
              <KpiCard
                label="Confirmados"
                value={data.confirmation.confirmed}
                intent="info"
              />
              <KpiCard
                label="Taxa de confirmação"
                value={`${data.confirmation.rate}%`}
                intent="info"
              />
              <KpiCard
                label="Presentes"
                value={data.participation.present}
                intent="success"
              />
              <KpiCard
                label="Taxa de participação"
                value={`${data.participation.rate}%`}
                intent="success"
              />
              <KpiCard
                label="Ausências"
                value={data.participation.absences}
                intent="danger"
              />
              <KpiCard
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
              <KpiCard
                label="Registadas"
                value={data.sessionAttendance.total}
              />
              <KpiCard
                label="Com check-in"
                value={data.sessionAttendance.checkedIn}
                intent="success"
              />
              <KpiCard
                label="Taxa"
                value={`${data.sessionAttendance.rate}%`}
                intent="info"
              />
            </div>
          </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
              ...
              <KpiCard
                label="Custo real"
                value={formatKz(data.costs.actualCost)}
              />
              <KpiCard
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
              ...
              <KpiCard
                ...
                value={
                  data.speakers.avgRating != null
                    ? `${data.speakers.avgRating} / 5`
                    : '—'
                }
                intent="accent"
              />
            ...
            <Card className="overflow-hidden p-0">
              ...
              <RankedList
                rows={data.byModality.map((m) => ({
                  label:
                    MODALITY_CFG[m.modalidade as keyof typeof MODALITY_CFG]
                      ?.label ?? m.modalidade,
                  count: m.count,
                }))}
                empty="Sem eventos"
          ...
          <Card className="overflow-hidden p-0">
            ...
            <RankedList
              rows={data.topEventsByParticipants.map((e) => ({
                label: e.title,
