...
function DetailView({ eventId, onBack }: { eventId: number; onBack: () => void }) {
  ...
  const checkInMutation = useApiMutation(
    ...
    {
      onSuccess: async () => { await refetch(); alert('✅ Check-in realizado! +20 XP'); },
  ...
  const feedbackMutation = useApiMutation(
    (payload: typeof feedback) => apiClient.post(`/events/${eventId}/feedback`, payload),
    {
      onSuccess: () => { setShowFeedback(false); alert('✅ Feedback enviado! Obrigado pela tua avaliação.'); },
      onError: (e) => alert(e.message),
    },
  );
          ...
          <div className="flex flex-col gap-2 flex-shrink-0">
            {event.status === 'PUBLISHED' && (
              <button onClick={handleJoin} disabled={joinMutation.isPending}
                ...
                {joinMutation.isPending ? '…' : event.isFull ? (event.waitlistEnabled ? 'Entrar na lista de espera' : 'Lotado') : '+ Inscrever-me'}
              </button>
            )}
      ...
      {showFeedback && (
        <div className="bg-amber-50 border border-amber-200 rounded-xl p-5 mb-5">
          ...
          <div className="space-y-4">
            ...
            <button onClick={handleFeedback} disabled={feedbackMutation.isPending}
