...
const INITIAL_DOC_FILTERS: DocFilters = {
  search: '',
  category: '',
  sensitivity: '',
  tag: '',
          ...
          >
            <div className="p-6 border-b border-gray-100">
              <div className="flex items-start justify-between">
                <div>
                  ...
                  <span
                    className={`mt-1 inline-block text-xs px-2.5 py-0.5 rounded-full font-medium ${CATEGORY_CONFIG[selectedDoc.category]?.color}`}
                  >
            ...
            <div className="p-6 space-y-4">
              {[
                ...
                {
                  label: 'Criado por',
                  value: selectedDoc.createdBy?.name ?? '—',
                },
                {
                  label: 'Proprietário',
                  value: selectedDoc.owner?.name ?? '—',
                },
                {
                  label: 'Validade',
                  value: selectedDoc.expiresAt
                ...
                },
                {
                  ...
                  value: selectedDoc.retentionUntil
                    ? new Date(selectedDoc.retentionUntil).toLocaleDateString(
                        'pt-PT',
                      )
                    : '—',
                },
                {
                  label: 'Downloads',
                  value: String(selectedDoc.downloadCount ?? 0),
                },
                {
                  label: 'Versões',
                  value: String(selectedDoc._count?.versions ?? 1),
                },
              ].map((row) => (
                <div
                  key={row.label}
